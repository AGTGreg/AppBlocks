...
- [ ] T001 Create `.benchmarks/` directory (local baseline store, gitignored)
- [ ] T002 Add `.benchmarks/baseline.json` initialization script placeholder in `scripts/benchmark.js`
- [ ] T003 Ensure gitignore includes `.benchmarks/` entry
- [ ] T004 Add documentation stubs in `docs/changelog.md` for upcoming features

## Phase 2: Foundational

Purpose: Prepare failing tests (TDD) and scaffolds before implementing logic.

- [ ] T005 Create test file `tests/core/events.selectors-with-spaces.test.js` with failing tests for descendant selector delegation
- [ ] T006 Create test file `tests/placeholders/delimiters.test.js` with failing tests for custom delimiters & filters
- [ ] T007 Create test file `tests/benchmark/render-benchmark.test.js` asserting output shape (10 samples, mean)
- [ ] T008 Add benchmark runner skeleton `scripts/benchmark.js` (exports `runBenchmark()` returning placeholder structure)
- [ ] T009 Update `docs/api.md` with placeholder section for `delimiters` config key (to be finalized post-implementation)
...
- [ ] T010 [US1] Implement event key parsing change in `src/core.js` (split first space only)
...
- [ ] T014 [US2] Implement delimiters config reading in `src/core.js` (store on instance)
...
- [ ] T026 Finalize `docs/changelog.md` entries summarizing added features
...
- [ ] T029 Ensure all tests pass and remove any redundant temporary code/comments
- [ ] T030 Add example snippet for benchmark output to `quickstart.md`
